...
class ReportsSubscriptions extends Component {
    ...
    getData = () => {
        ...
        func.get('subscriptions', { orderby: 'crdate_asc', between: `crdate_${moment(between[0]).format('YYYY-MM-DD')}_${moment(between[1]).format('YYYY-MM-DD')}`,  limit: `${step},${limit}` }).then(res => {
            this.setState({ loading: false });
            if (res.status === 200) {
