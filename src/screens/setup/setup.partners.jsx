...
class SetupPartnersScreen extends Component {
    ...
    componentDidMount() {
        this.props.setPageTitle('Setup: Manage partners');
        this.getData();
    }
    ...
    getData = () => {
        ...
        func.post('partners', { status: istatus, name: `%${ititle}%`, limit: `${step},${limit}` }).then(res => {
    ...
    removeImage = () => {
        ...
        if (modalAction === 'insert') {
            ...
            this.setState({ image: row.logo_link, file: null });
        }
    }
    nextPrev = (e) => {
    ...
    form = (row = {}) => {
        if (row.id) {
            this.setState({ modalTitle: 'Edit partner', modalAction: 'update', row, image: row.logo ? row.logo_link : defaultImage, modalShow: !this.state.modalShow });
        } else {
            this.setState({ modalTitle: 'Add partner', modalAction: 'insert', row, image: defaultImage, modalShow: !this.state.modalShow });
        }
    }
    submit = () => {
        ...
        validateFields((err, v) => {
            if (!err) {
                ...
                if (file) {
                    func.postFile('upload', { folder: 'partners', 'file[0]': file, filename: v.title }).then(res => {
                        if (res.status === 200) {
                            image = res.result;
    ...
    submitGo = (v, image) => {
        ...
        v['id'] = id;
        v['logo'] = image;
        func.post(`partners/${modalAction}`, v).then((res) => {
    ...
    delete = (row) => {
        ...
        func.post(`partners/delete`, { id }).then((res) => {
            this.setState({ submitting: false });
            if (res.status === 200) {
                        ...
                        <div className="jumbotron">
                            <Form hideRequiredMark={false}>
                                <div className="row row-xs">
                                    ...
                                    <div className="col-4">
                                        <Input size="large" placeholder="Filter by name" disabled={loading} onPressEnter={this.filter} onChange={e => this.formChange(e, 'ititle')} />
                                    </div>
                                    <div className="col-2">
                                            ...
                                            <tr key={row.id} className={edited === row.id ? 'animated shake bg-gray-100' : ''}>
                                                ...
                                                <td width="6%">
                                                    <a href={row.link} target="_blank" rel="noopener noreferrer">
                                                        <img className="img-thumbnail" src={row.logo ? row.logo_link : defaultImage} alt={row.name} width="100%" />
                                                    </a>
                                                </td>
                                                <td>{row.name}</td>
...
const SetupPartners = Form.create()(SetupPartnersScreen);
export default SetupPartners;